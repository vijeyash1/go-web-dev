...
func (rh RunnersController) CreateRunner(ctx *gin.Context) {
	...
	if err != nil {
		log.Println("Error while unmarshalling the create runner request body", err)
		ctx.AbortWithError(http.StatusInternalServerError, err)
		return
	}
...
func (rh RunnersController) UpdateRunner(ctx *gin.Context) {
	body, err := io.ReadAll(ctx.Request.Body)
	if err != nil {
		log.Println("Error while reading the update runner request body", err)
		ctx.AbortWithError(http.StatusInternalServerError, err)
		return
	}
	var runner models.Runner
	err = json.Unmarshal(body, &runner)
	if err != nil {
		log.Println("Error while unmarshalling the update runner body", err)
		ctx.AbortWithError(http.StatusInternalServerError, err)
		return
	}
